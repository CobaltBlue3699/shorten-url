...
import { KEYCLOAK_CONNECT_OPTIONS, KEYCLOAK_INSTANCE, PolicyEnforcementMode } from '../constants';
...
export class ResourceGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const scopes = this.reflector.get<string[]>(META_SCOPES, context.getHandler());
    const isUnprotected = this.reflector.getAllAndOverride<boolean>(META_UNPROTECTED, [
      context.getClass(),
      context.getHandler(),
    ]);
    const enforcerOpts = this.reflector.getAllAndOverride<KeycloakConnect.EnforcerOptions>(META_ENFORCER_OPTIONS, [
      context.getClass(),
      context.getHandler(),
    ]);

    // Default to permissive
    const pem = this.keycloakOpts.policyEnforcement || PolicyEnforcementMode.PERMISSIVE;
    ...
    if (!scopes) {
      if (shouldAllow) {
        KeycloakConnectModule.logger.verbose(`Route has no @Scope defined, request allowed due to policy enforcement`);
      } else {
        KeycloakConnectModule.logger.verbose(`Route has no @Scope defined, request denied due to policy enforcement`);
      }
      return shouldAllow;
    }

    KeycloakConnectModule.logger.verbose(`Protecting resource [ ${resource} ] with scopes: [ ${scopes} ]`);
    ...
    if (!request.user && isUnprotected) {
      KeycloakConnectModule.logger.verbose(`Route has no user, and is public, allowed`);
      return true;
    }
    ...
    if (!isAllowed) {
      KeycloakConnectModule.logger.verbose(`Resource [ ${resource} ] denied to [ ${user} ]`);
    } else {
      KeycloakConnectModule.logger.verbose(`Resource [ ${resource} ] granted to [ ${user} ]`);
    }
